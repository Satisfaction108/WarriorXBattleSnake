...
def calculate_center_control(pos: dict, board_width: int, board_height: int) -> float:
    """
    Calculate how close a position is to the center of the board.
    Returns a value from 0 (edge) to 1 (center).
    Center control is CRITICAL in 1v1 endgames!
    """
    ...
    max_dist = center_x + center_y  # Maximum possible distance from center

    # Normalize to 0-1 (1 = at center, 0 = at corner)
    center_score = 1.0 - (dist_from_center / max_dist)
    return center_score


def detect_cutoff_opportunity(my_pos: dict, opponent: dict, game_state: dict) -> dict:
    """
    Detect if we can cut off an opponent's path to food or safety.
    This is a CRITICAL offensive tactic!

    Returns:
        {
            "can_cutoff": bool,
            "cutoff_value": int,  # How valuable this cutoff is
            "target_type": str,   # "food", "safety", "escape"
        }
    """
    board_width = game_state["board"]["width"]
    board_height = game_state["board"]["height"]
    ...
    # If opponent is low on health, cutting off food is DEADLY
    if opponent_health < 30 and food_list:
        # Find food closest to opponent
        closest_food = None
        closest_dist = float('inf')

        for food in food_list:
            dist = manhattan_distance(opponent_head, food)
            if dist < closest_dist:
                closest_dist = dist
                closest_food = food

        if closest_food:
            # Check if we can reach a position that blocks their path
            my_dist_to_food = manhattan_distance(my_pos, closest_food)
            opponent_dist_to_food = manhattan_distance(opponent_head, closest_food)

            # If we can reach the food first or at the same time, we can deny it!
            if my_dist_to_food <= opponent_dist_to_food:
                cutoff_value = 100000 + (30 - opponent_health) * 5000
                return {
                    "can_cutoff": True,
    ...
    opponent_flood = flood_fill(opponent_head, game_state, max_depth=10)
    if opponent_flood < 15:  # They have limited space
        # Check if moving to my_pos reduces their space further
        # This is complex, so we'll use a simple heuristic:
        # If we're adjacent to their limited space, we're cutting them off
        dist_to_opponent = manhattan_distance(my_pos, opponent_head)
        if dist_to_opponent <= 3:
            cutoff_value = 80000 + (15 - opponent_flood) * 4000
            return {
                "can_cutoff": True,
                "cutoff_value": cutoff_value,
                "target_type": "escape",
...
def calculate_food_denial_value(my_pos: dict, game_state: dict) -> int:
    """
    Calculate the value of denying food to opponents.
    When we're the longest snake, denying food is a winning strategy!
    """
    my_length = len(game_state["you"]["body"])
    food_list = game_state["board"]["food"]
    opponents = [s for s in game_state["board"]["snakes"] if s["id"] != game_state["you"]["id"]]

    if not opponents or not food_list:
        return 0

    # Check if we're the longest snake
    longest_opponent = max(opponents, key=lambda s: len(s["body"]))
    longest_opponent_length = len(longest_opponent["body"])

    if my_length <= longest_opponent_length:
        # We're not winning, don't focus on denial
        return 0

    # We're the longest! Denying food is a winning strategy
    denial_value = 0

    for food in food_list:
        my_dist = manhattan_distance(my_pos, food)

        # Find closest opponent to this food
        closest_opponent_dist = float('inf')
        closest_opponent = None

        for opponent in opponents:
            opp_head = opponent["body"][0]
            opp_dist = manhattan_distance(opp_head, food)
            if opp_dist < closest_opponent_dist:
                closest_opponent_dist = opp_dist
                closest_opponent = opponent

        # If we can reach the food before or at the same time as opponent
        if my_dist <= closest_opponent_dist and closest_opponent:
            # Denial value increases if opponent is low on health
            opponent_health = closest_opponent["health"]
            if opponent_health < 40:
                denial_value += 60000 + (40 - opponent_health) * 2000
            else:
                denial_value += 30000
...
def evaluate_move(direction: str, game_state: dict, use_prediction: bool = True) -> dict:
    ...
    # ========================================================================
    # PHASE 9: 1v1 ENDGAME STRATEGY - CRITICAL FOR WINNING!
    # ========================================================================

    num_opponents = len([s for s in game_state["board"]["snakes"] if s["id"] != game_state["you"]["id"]])

    if num_opponents == 1:
        # 1v1 ENDGAME - This is where games are won or lost!
        opponent = [s for s in game_state["board"]["snakes"] if s["id"] != game_state["you"]["id"]][0]
        ...
        my_center_control = calculate_center_control(new_head, board_width, board_height)
        opponent_center_control = calculate_center_control(opponent_head, board_width, board_height)

        # Strategy 1: If we're LONGER - BE AGGRESSIVE!
        if length_diff >= 3:
            # We have significant advantage - DOMINATE!

            # Center control is CRITICAL
            if my_center_control > opponent_center_control:
                center_bonus = 150000
                score += center_bonus
                reasons.append(f"👑 1v1 CENTER CONTROL: Dominating center! (+{center_bonus})")
            else:
                # We should be controlling center!
                center_penalty = 80000
                score -= center_penalty
                reasons.append(f"⚠️  1v1 LOSING CENTER: Move to center! (-{center_penalty})")

            # Food denial - starve them out!
            denial_value = calculate_food_denial_value(new_head, game_state)
            if denial_value > 0:
                score += denial_value
                reasons.append(f"🚫 1v1 FOOD DENIAL: Starving opponent! (+{denial_value})")

            # Cutoff opportunities
            cutoff = detect_cutoff_opportunity(new_head, opponent, game_state)
            if cutoff["can_cutoff"]:
                score += cutoff["cutoff_value"]
                reasons.append(f"✂️  1v1 CUTOFF: Blocking {cutoff['target_type']}! (+{cutoff['cutoff_value']})")

            # Move toward opponent to pressure them
            dist_to_opponent = manhattan_distance(new_head, opponent_head)
            if dist_to_opponent <= 5:
                pressure_bonus = 60000
                score += pressure_bonus
                reasons.append(f"💪 1v1 PRESSURE: Closing in on opponent! (+{pressure_bonus})")

        # Strategy 2: If we're SHORTER - SURVIVE AND WAIT!
        elif length_diff <= -3:
            # We're at disadvantage - AVOID and SURVIVE!

            # Avoid opponent at all costs
            dist_to_opponent = manhattan_distance(new_head, opponent_head)
            if dist_to_opponent <= 3:
                danger_penalty = 120000
                score -= danger_penalty
                reasons.append(f"🏃 1v1 RETREAT: Too close to bigger opponent! (-{danger_penalty})")
            elif dist_to_opponent >= 6:
                safety_bonus = 80000
                score += safety_bonus
                reasons.append(f"✅ 1v1 SAFE DISTANCE: Keeping away from threat (+{safety_bonus})")

            # Prioritize survival and space
            if trap_analysis["available_space"] > 30:
                space_bonus = 100000
                score += space_bonus
                reasons.append(f"🌊 1v1 SPACE: Plenty of room to maneuver (+{space_bonus})")

            # Get food when safe to grow
            if food_list and my_health < 70:
                # Find safest food (farthest from opponent)
                safest_food_dist = 0
                for food in food_list:
                    if coords_equal(new_head, food):
                        food_dist_from_opponent = manhattan_distance(food, opponent_head)
                        if food_dist_from_opponent >= 5:
                            safe_food_bonus = 140000
                            score += safe_food_bonus
                            reasons.append(f"🍎 1v1 SAFE FOOD: Growing safely! (+{safe_food_bonus})")

        # Strategy 3: If we're EQUAL - CONTROL CENTER AND FORCE MISTAKES!
        else:
            # Equal length - it's a chess match!

            # Center control is EVERYTHING in equal matchups
            if my_center_control > opponent_center_control + 0.1:
                center_bonus = 180000
                score += center_bonus
                reasons.append(f"♟️  1v1 EQUAL CENTER: Controlling board! (+{center_bonus})")
            elif my_center_control < opponent_center_control - 0.1:
                center_penalty = 100000
                score -= center_penalty
                reasons.append(f"⚠️  1v1 EQUAL LOSING: Opponent controls center! (-{center_penalty})")

            # In equal matchups, force opponent to edges
            opponent_edge_dist = min(
                opponent_head["x"],
                opponent_head["y"],
            ...
            if opponent_edge_dist <= 2:
                # Opponent is near edge - CAPITALIZE!
                edge_advantage = 120000
                score += edge_advantage
                reasons.append(f"🎯 1v1 EQUAL EDGE: Opponent trapped near edge! (+{edge_advantage})")

            # Maintain optimal distance (not too close, not too far)
            dist_to_opponent = manhattan_distance(new_head, opponent_head)
            if 3 <= dist_to_opponent <= 5:
                optimal_dist_bonus = 70000
                score += optimal_dist_bonus
                reasons.append(f"📏 1v1 EQUAL DISTANCE: Optimal spacing (+{optimal_dist_bonus})")

            # Cutoff opportunities are CRITICAL in equal matchups
            cutoff = detect_cutoff_opportunity(new_head, opponent, game_state)
            if cutoff["can_cutoff"]:
                score += cutoff["cutoff_value"] * 1.5  # Extra valuable in equal matchup
                reasons.append(f"✂️  1v1 EQUAL CUTOFF: Game-winning move! (+{int(cutoff['cutoff_value'] * 1.5)})")

    # ========================================================================
    # PHASE 10: MULTI-OPPONENT CUTOFF & DENIAL
    # ========================================================================

    elif num_opponents >= 2:
        # In multi-opponent games, cutoff and denial are still valuable
        # but less critical than in 1v1

        # Check cutoff opportunities against each opponent
        total_cutoff_value = 0
        for opponent in [s for s in game_state["board"]["snakes"] if s["id"] != game_state["you"]["id"]]:
            cutoff = detect_cutoff_opportunity(new_head, opponent, game_state)
            if cutoff["can_cutoff"]:
                total_cutoff_value += cutoff["cutoff_value"] // 2  # Half value in multi-player

        if total_cutoff_value > 0:
            score += total_cutoff_value
            reasons.append(f"✂️  MULTI CUTOFF: Blocking opponents! (+{total_cutoff_value})")

        # Food denial when winning
        denial_value = calculate_food_denial_value(new_head, game_state)
        if denial_value > 0:
            score += denial_value // 2  # Half value in multi-player
            reasons.append(f"🚫 FOOD DENIAL: Denying opponents! (+{denial_value // 2})")
...
def move(game_state: typing.Dict) -> typing.Dict:
    ...
    # ANTI-PREDICTABILITY: If top moves have VERY similar scores, randomize to avoid patterns
    # TIGHTENED: Changed from 50 to 5 points - only randomize when moves are truly equal
    # This prevents choosing suboptimal moves and makes the snake more consistent
    import random
    best_score = move_evaluations[0]["score"]
    similar_moves = [m for m in move_evaluations if abs(m["score"] - best_score) < 5]

    if len(similar_moves) > 1:
        print(f"🎲 Multiple equal moves (within 5 points): {[m['direction'] for m in similar_moves]}")
        best_move = random.choice(similar_moves)
        print(f"   Randomly chose: {best_move['direction'].upper()}")
    else:
        best_move = move_evaluations[0]
        print(f"🎯 Clear best move: {best_move['direction'].upper()} (score: {best_move['score']})")
